...
class RNN(nn.Module):
    def __init__(self, vocab_size, embedding_dim,  n_filters, filter_sizes,Hidden_cnn,Dropout_cnn,hidden_dim, output_dim, n_layers, 
        ...
        self.fc1 = nn.Linear(250+hidden_dim * 2, 128)
        ...
        self.fc_cnn=nn.Linear(len(filter_sizes) * n_filters,250)
    ...
    def forward(self, text, text_lengths):
        ...
        ######### removed F.relu from conved
        conved = [conv(embedded_cnn).squeeze(3) for conv in self.convs]
        pooled = [F.max_pool1d(conv, conv.shape[2]).squeeze(2) for conv in conved]
        pooled=self.relu(pooled)
        pooled=self.dropout(self.fc_cnn(pooled))
        pooled=self.relu(pooled)
        #unpack sequence
